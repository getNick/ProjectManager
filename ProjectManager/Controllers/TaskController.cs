...
namespace ProjectManager.Controllers
{
    public class TaskController : Controller
    {
        ...
        public IActionResult Index(int? selectedDepartmentId,int? selectedTeamId)
        {
            ...
            {
                vm.AllTeams = _db.Teams.Include(x => x.Department).Include(x => x.Tasks).ThenInclude(x => x.Assignee).ThenInclude(x => x.User).Where(x => x.Department.Id == selectedDepartmentId).ToList();
                vm.SelectedTeamId = selectedTeamId;
                ...
                vm.SelectedDepartmentId = vm.AllTeams.FirstOrDefault(x => x.Id == selectedTeamId).Department.Id;
                return View(vm);
            }
            ...
            {
                vm.AllTeams = _db.Teams.Include(x=>x.Department).Include(x=>x.Tasks).ThenInclude(x=>x.Assignee).ThenInclude(x=>x.User).Where(x=>x.Department.Id==selectedDepartmentId).ToList();
                vm.SelectedDepartmentId = selectedDepartmentId;
                vm.CustomList = vm.AllDepartments?.FirstOrDefault(x => x.Id == selectedDepartmentId).Tasks;
